			...
			foreach ($_FILES as $file) {
				if ($file['size'] > 0) {
					...
					if (!in_array($ext, $allowed_ext)) {
						...
						// move_uploaded_file($file['tmp_name'], "../wallpaper/".time().'__'.$file['name']);
						// move_uploaded_file($file['tmp_name'], "../wallpaper/".time().'.'.$ext);
			...
			foreach ($_FILES as $file) {
				if ($file['size'] > 0) {
					...
					if (!in_array($ext, $allowed_ext)) {
						...
						// move_uploaded_file($file['tmp_name'], "../img/logo.".$ext);//ganti logo masih kesimpen  di cache browser
					}
				}
				$i++;
			}
